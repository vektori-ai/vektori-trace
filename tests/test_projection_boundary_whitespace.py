...
class TestInterleavedToolCalls:
    """Tool calls emitted INSIDE the reasoning block.

    Observed on u000-task76-seed0@9#0 (2026-08-29): three `<tool_call>` blocks
    in the middle of an 8,611-byte reasoning span. The tool JSON is excluded
    (DeepSeek never sees Hermes serialization), which leaves the reasoning
    discontiguous -- a hole, not a shortened edge. A single byte range would
    either swallow the tool JSON or claim bytes no supervised token covers, and
    the scorer refused the whole payload: 8,324 bytes lost to save 287.

    Each contiguous run is now its own span, aligned and scored separately.
    """

    TC = '<tool_call>{"name": "f", "arguments": {}}</tool_call>'

    def test_reasoning_split_into_segments(self):
        raw = f"<think>\nFirst part.\n{self.TC}\nSecond part.\n</think>Done."
        toks = _toks("<think>", "\n", "First part", ".\n", self.TC, "\n",
                     "Second part", ".\n", "</think>", "Done.")
        p = project_action(raw, toks)
        labels = [s.label for s in p.payloads if s.kind == "reasoning"]
        assert len(labels) >= 2, f"expected multiple segments, got {labels}"
        assert "reasoning" in labels and "reasoning#1" in labels

    def test_each_segment_matches_its_tokens(self):
        """The invariant the scorer checks, per segment."""
        raw = f"<think>\nFirst part.\n{self.TC}\nSecond part.\n</think>Done."
        toks = _toks("<think>", "\n", "First part", ".\n", self.TC, "\n",
                     "Second part", ".\n", "</think>", "Done.")
        p = project_action(raw, toks)
        ...
        for sp in p.payloads:
            idx = sorted(i for i, k in p.supervised.items() if k == sp.label)
            joined = b"".join(toks[i] for i in idx)
            assert joined == raw_b[sp.byte_start:sp.byte_end], (
                f"{sp.label}: tokens do not reassemble the span")

    def test_tool_call_carries_no_credit(self):
        raw = f"<think>\nFirst.\n{self.TC}\nSecond.\n</think>Done."
        toks = _toks("<think>", "\n", "First", ".\n", self.TC, "\n",
                     "Second", ".\n", "</think>", "Done.")
        p = project_action(raw, toks)
        assert 4 not in p.supervised, "the tool call must not be supervised"

    def test_no_span_carries_boundary_whitespace(self):
        from vektori_trace.tau2.live_projection import normalize_payload
        raw = f"<think>\nA.\n{self.TC}\nB.\n{self.TC}\nC.\n</think>D."
        toks = _toks("<think>", "\n", "A", ".\n", self.TC, "\n", "B", ".\n",
                     self.TC, "\n", "C", ".\n", "</think>", "D.")
        ...
        raw_b = raw.encode("utf-8")
        for sp in p.payloads:
            payload = raw_b[sp.byte_start:sp.byte_end].decode()
            assert payload == normalize_payload(payload)[0], (
                f"{sp.label} carries boundary whitespace: {payload!r}")

    def test_single_segment_keeps_the_bare_label(self):
        """Backwards compatible: an uninterrupted payload is still 'reasoning'."""
        raw = "<think>\nJust reasoning.\n</think>Done."
        toks = _toks("<think>", "\n", "Just reasoning", ".\n", "</think>",
                     "Done.")
        p = project_action(raw, toks)
        labels = {s.label for s in p.payloads}
        assert "reasoning#1" not in labels
